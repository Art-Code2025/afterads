...
async function generateExcelInvoice(order) {
  ...
  worksheet.getCell('D8').value = 'العنوان:';
  ...
  if (order.city && order.city !== 'غير محدد') {
    addressParts.push(order.city);
  }
  const address = addressParts.length > 0 ? addressParts.join(', ') : 'غير محدد';
...
async function generateMonthlyStatsExcel(year, month) {
  ...
  orders.forEach((order, index) => {
    ...
    const address = addressParts.length > 0 ? addressParts.join(', ') : 'غير محدد';
    ordersSheet.getCell(row, 4).value = address;
    ordersSheet.getCell(row, 4).style = rowStyle;
    
    ordersSheet.getCell(row, 5).value = order.city || 'غير محدد';
    ordersSheet.getCell(row, 5).style = rowStyle;
    
    ordersSheet.getCell(row, 6).value = `${(order.total || 0).toFixed(2)} SAR`;
    ordersSheet.getCell(row, 6).style = { ...rowStyle, alignment: { horizontal: 'right' } };
    
    ordersSheet.getCell(row, 7).value = getStatusText(order.status);
    ordersSheet.getCell(row, 7).style = rowStyle;
    
    ordersSheet.getCell(row, 8).value = formatArabicDate(order.createdAt);
    ordersSheet.getCell(row, 8).style = rowStyle;
    
    ordersSheet.getCell(row, 9).value = formatArabicTime(order.createdAt);
    ordersSheet.getCell(row, 9).style = rowStyle;
